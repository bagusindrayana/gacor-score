<script>
    ...
    async function fecthData(pemprov) {
        ...
        await fetch(`/data/${pemprov}.json`)
            ...
            .then((res) => {
                ...
                result.hostname = res.map((data) => new URL(atob(data.link)).hostname);
    ...
    const jumlahJudiLabel = (judi) => {
        if (judi >= 99){
            return "<span class='text-red-500 font-bold'>"+judi.toString()+"+</span>"
        } else if(judi >= 50){
            return "<span class='text-orange-500 font-bold'>"+judi.toString()+"+</span>"
        } else if(judi > 0){
            return "<span class='text-yellow-500 font-bold'>"+judi.toString()+"</span>"
        } else {
            return "<span class='text-green-500 font-bold'>"+judi.toString()+"</span>"
        }
    };
    ...
    const jumlahHostnameLabel = (host) => {
        if (host >= 10){
            return "<span class='text-red-500 font-bold'>"+host.toString()+"</span>"
        } else if(host >= 5){
            return "<span class='text-orange-500 font-bold'>"+host.toString()+"+</span>"
        } else if(host > 0){
            return "<span class='text-yellow-500 font-bold'>"+host.toString()+"</span>"
        } else {
            return "<span class='text-green-500 font-bold'>"+host.toString()+"</span>"
        }
    };
    ...
    onMount(async () => {
        ...
        datas.forEach((data,i) => {
            newData.push(["<a href='https://www.google.com/search?q=site%3A"+data.domain+"+%22slot+online%22+OR+site%3A"+data.domain+"+%22gacor%22%27&num=100' class='underline'>"+data.domain+"</a>", "<span class='font-bold open-judi cursor-pointer' data-index='"+i.toString()+"'>"+jumlahJudiLabel(data.judi)+"</span>", "<span class='font-bold open-domain cursor-pointer' data-index='"+i.toString()+"'>"+jumlahHostnameLabel(data.hostname.length)+"</span>"]);
        });
        datatable.insert({
        ...
        document.querySelectorAll('span.open-domain').forEach((element) => {
            ...
            element.addEventListener('click', (e) => {
                ...
                modalSubTitle = "bagian yang disusupi bisa berupa komentar, hidden link, atau bagian yang datanya dinamis";
                selectedData = datas[parseInt(e.currentTarget.getAttribute('data-index'))];
                let lists = "<ol class='list-decimal'>";
                for (let index = 0; index < selectedData.hostname.length; index++) {
        ...
        document.querySelectorAll('span.open-judi').forEach((element) => {
            ...
            element.addEventListener('click', (e) => {
                ...
                selectedData = datas[parseInt(e.currentTarget.getAttribute('data-index'))];
                let lists = "<ol class='list-decimal'>";
                for (let index = 0; index < selectedData.links.length; index++) {
...
<div class="text-gray-200 w-full p-6">
    {#if showModal}
  <Modal title="{modalTitle}" message="{modalMessage}" subTitle={modalSubTitle} on:close={closeModal} />
{/if}
    ...
    <div class="dark flex flex-col w-full min-h-screen p-4 md:p-6 text-white">
        <header class="flex flex-col mb-4">
            ...
            <p>Berdasarkan google dork <span class="underline italic">site:nama_provinsi.go.id "slot online" OR site:nama_provinsi.go.id "gacor"</span></p>
            <p class="text-yellow-400">list ini sifatnya tidak realtime, bisa saja halaman yang ter index google sudah di fix</p>
            <p class="text-yellow-400">Terakhir diupdate tanggal : 23/04/2024</p>
        </header>
